...
guidedTour.createTour = function(element, stages) {
    ...
    tutorialMessageContainer.setAttribute('Stage', 0);
    ...
    tutorialMessage.textContent = stages[0].message;
    ...
    guidedTour.animateToElement(guidedTour.stages[0].targetElement, guidedTour.stages[0].position);

}

guidedTour.endTour = function() {
	console.log('end tour');
	window.localStorage.setItem('guidedTourFinished', 'true');
...
guidedTour.nextTutorialStage = function() {
	...
	if ($(guidedTour.stages[nextStage].targetElement).length > 0) {
