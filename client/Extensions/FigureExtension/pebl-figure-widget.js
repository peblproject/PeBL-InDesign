...
figure.createFigure = function(insertID, img, title, caption, zoomable) {
	...
	figureWrapper.setAttribute('data-theme-target', insertID);
	...
	if (img.constructor === Array) {
		for (image in img) {
			...
			imageElement.alt = caption.replace(/&lt;br\/>/g, '<br/>').replace('&',' and ');
	...
	paragraph.innerHTML = caption.replace(/&lt;br\/>/g, '<br/>').replace('&',' and ');
    ...
    insertLocation.parentNode.insertBefore(figureWrapper, insertLocation);
