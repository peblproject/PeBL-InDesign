...
jQuery(document).ready(function() {
	jQuery('.peblExtension[data-peblextension="askExpert"], .peblExtension[data-peblExtension="askExpert"]').each(function () {
...
askExpert.createAskExpertExpertModal = function(id, experts, mailto, mailtoSubject, mailtoBody) {
	globalPebl.user.getUser(function(userProfile) {
		if (userProfile) {
			...
			expertsModal.setAttribute('data-theme-target', id);
			...
			askExpertModalDescription.textContent = 'Have questions about the material? Ask an expert here.';
			...
			for (var expert of experts) {
				if (expert.enabled) {
			        ...
			        var contactIcon = 'fa-envelope';
			        if (expert.phone)
			        	contactIcon = 'fa-phone';
			        else if (expert.url)
			        	contactIcon = 'fa-external-link-alt';
			        expertContactIcon.classList.add('fa', contactIcon, 'askExpertIcon');
					...
					(function(expert) {
						expertContainer.addEventListener('click', function(evt) {
							if (expert.email) {
								...
								window.open('tel:+' + expert.phone, '_blank');
							} else if (expert.url) {
								if (!expert.url.match(/^http?:\/\//i) || !expert.url.match(/^https?:\/\//i)) {
							        expert.url = 'http://' + expert.url;
							    }
								window.open(expert.url, '_blank');
							} else {
...
askExpert.createAskExpertButton = function(insertID, buttonText, experts, mailto, mailtoRecipient, mailtoSubject, mailtoBody) {
	var askExpertButtonWrapper = document.createElement('div');
	askExpertButtonWrapper.setAttribute('data-theme-target', insertID);
	...
	askExpertButtonWrapper.appendChild(askExpertButton);

	insertLocation = document.getElementById(insertID);

    insertLocation.parentNode.insertBefore(askExpertButtonWrapper, insertLocation);
