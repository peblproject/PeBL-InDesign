...
dataEntry.insertExtensions = function() {
    jQuery('.dataEntryExtension, .peblExtension[data-peblextension="dataentry"], .peblExtension[data-peblExtension="dataentry"], .peblExtension[data-peblExtension="dataentrypoll"], .peblExtension[data-peblExtension="dataentrytext"]').each(function() {        
...
dataEntry.createMultipleChoiceEntry = function (id, form, activeEntry, useGraphView) {
    ...
    for (var i = 0; i < form.responses.length; i++) {
        ...
        multiChoiceButton.innerHTML = form.responses[i].replace(/&lt;br\/>/g, '<br/>').replace('&',' and ');
        ...
        if (useGraphView && useGraphView === 'true') {
            ...
            multiChoiceResponseText.innerHTML = form.responses[i].replace(/&lt;br\/>/g, '<br/>').replace('&',' and ');        
            ...
            multiChoiceResponseTextTeam.innerHTML = form.responses[i].replace(/&lt;br\/>/g, '<br/>').replace('&',' and ');
            ...
            multiChoiceResponseTextClass.innerHTML = form.responses[i].replace(/&lt;br\/>/g, '<br/>').replace('&',' and ');
            ...
            subFormSubmit.on('click', function () {
                ...
                if (validForm) {
                    ...
                    if (globalReadium) {
                        ...
                        finalMessage.cfi = bookmark.contentCFI;
                        finalMessage.idRef = bookmark.idref;
                    }
...
dataEntry.createDataEntry = function (insertID, question, id, forms, sharing, displayMode, polling, useConfig, closeable) {
    ...
    var makeDataEntry = function(insertID, question, id, forms, sharing, displayMode, polling, useConfig, closeable, userProfile, programID, group) {
        var dataEntryID;
            ...
            var privateAndEmailSharing = false;
            ...
            } else if (sharing === 'private+email') {
                dataEntryID = id;
                privateAndEmailSharing = true;
            } else if (sharing === 'team+class') {
                dataEntryID = id;
                //Handle it later...
            ...
            dataEntryWrapper.setAttribute('data-theme-target', insertID);
            if (sharing === 'private') {
            ...
            questionParagraph.classList.add('dataEntryMainPrompt');
                    ...
                    globalPebl.storage.getCurrentBook(function(book) {
                        ...
                        if (group) {
                            ...
                            if (globalReadium) {
                                ...
                                artifactMessage.cfi = bookmark.contentCFI;
                                artifactMessage.idRef = bookmark.idref;
                            }
            ...
            var variableFormSubmitEmail = jQuery('<button class="dataEntryFormSubmitEmail edit">Email</button>');
            globalPebl.storage.getCurrentBook(function(book) {
                variableFormSubmitEmail[0].addEventListener('click', function(evt) {
                    var message = dataEntry.getFormData(formElement, newDataEntry, 'Private');
                    if (message != null) {
                        var mailtoLink = 'mailto:?subject=' + encodeURI(book) + '&body=';
                        mailtoLink += encodeURI(message[0].mainPrompt + '\r\n\r\n');
                        for (var msg of message) {
                            mailtoLink += encodeURI(msg.prompt + '\r\n');
                            mailtoLink += encodeURI(msg.text + '\r\n\r\n');
                        }
            ...
            if (sharing === 'private') {
                ...
                var privateMessageHandle = dataEntry.dataMessageHandler(dataEntryID);
                if (globalPebl)
                    globalPebl.subscribeThread(dataEntryID, false, privateMessageHandle, {isPrivate: true});
                ...
                if (userProfile.currentTeam) {
                    var teamMessageHandle = dataEntry.dataMessageHandler(dataEntryID);
                    if (globalPebl)
                        globalPebl.subscribeThread(dataEntryID, false, teamMessageHandle, {groupId: userProfile.currentTeam});
                }

                if (userProfile.currentClass) {
                    var classMessageHandle = dataEntry.dataMessageHandler(dataEntryID);
                    if (globalPebl)
                        globalPebl.subscribeThread(dataEntryID, false, classMessageHandle, {groupId: userProfile.currentClass});
                }
            ...
            } else if (privateAndEmailSharing) {
                header.appendChild(privateViewModeButton);

                jQuery(formFooter).append(variableFormSubmitPrivate);
                jQuery(formFooter).append(variableFormSubmitEmail);
    ...
    if (!globalPebl)
        ...
        globalPebl.user.getUser(function(userProfile) {
            if (!userProfile)
                makeDataEntry(insertID, question, id, forms, sharing, displayMode, polling, useConfig, closeable, {identity: 'temp', currentTeam: 'temp', currentClass: 'temp'}, null, null);
            else if (programID)
...
dataEntry.getFormData = function (formElement, newDataEntry, responseType, getMarkedResponses) {
    ...
    if (getMarkedResponses) {
        ...
        if (validForm) {
            var mainPrompt = "";
            var mainPromptElement = jQuery(formElement).find('.dataEntryMainPrompt').first();
            if (mainPromptElement)
                mainPrompt = mainPromptElement.text();
            var messages = [];
            //Submit the form
